...
# ✅ RUN 4 VERIFIED FROM DISK — ACCEPTED as the campaign's final run; the smoke is DETERMINISTIC under `explore=False` by measurement; the P7.2b packet is next, then two sequential merge reviews (2026-09-15, ~19:05)

**Read, not relayed.** `output/p7_2b/`: 105 entries (`canary.json` new), `COMPLETE in 30s`, no `FAILED`; `canary.json`: facts `-32648.0 / 247.75089149261333 / true / 360`, 0.68 s, `git_commit ded56a0`, `git_dirty false`; `logs/canary.log` now carries run 3's line (Python repr) and run 4's (JSON); manifest 108 lines including both, `sha256sum -c` 108 OK. Run 4's artifact `92b1592d…`: `git_commit ded56a0` (the branch tip at launch; `git diff --name-only 5dfaaa2..ded56a0 -- offline tests` empty, so the code is `5dfaaa2`'s), **`git_dirty: false`** — E2's defect gone. Canary block: this run's 0.68 s with `observed` and `checked_against` (once in the whole artifact) and `probe_chunks_canary` = 0.89 s at `f31973e`, *not recorded in these chunks*. **Four-way diff computed by the coordinator:** `probe`, `statistics`, `targets`, `disjointness`, `subjects`, `rule_a_secondary_values`, `registered_in`, `scenario_key`, `format_version`, `what_this_does_not_say` byte-identical across runs 1 → 2 → 3 → 4; only `canary`, `git_commit`, `git_dirty`, `smoke` differ. **Smoke, run 3 vs run 4: the only differing field is `seconds`** — `n_decisions_in_support` 230 (`mappo1000`) and 269 (`mix50`) twice, at different commits and canary times, under `explore=False`: **E3 answered by measurement — deterministic; no argmax-tie finding.** The driver ran attended, through a pipe and `tee -a`: capture 1,249 bytes, no NULs, no truncation. Secured into `output/p7_2b_runs/` with shas verified after copy: `run4_92b1592d.json`, both smoke chunks, `canary.json`, `canary.log`, the driver stdout.

## The P7.2b packet — `docs/returns/P7.2b.md`, written by the implementer against `BRIEF_36` + Amendments A–E and every block after them
Committed **with** `docs/data/p7_2b_calibration.json` (run 4's, `92b1592d…`) in one commit on the branch; then `main` merged into the branch (docs only). It carries, beyond `docs/returns/TEMPLATE.md` and §5.5 of this brief: (1) the probe table's summary — means and maxes per k beside the six source statistics, the eight targets per subject with the registered one marked, each target's in-support position against both bounds; (2) the smoke's mechanics per run with the line *ATT deliberately not reported*, and the determinism measurement (230 / 269 in runs 3 and 4); (3) **the canary of every run** — all four lines quoted verbatim with their source files and shas (runs 1–3 from the `/tmp` captures secured as `tmp_capture_p72b_*.log`; run 4 from `canary.json`), with the sentence: *the probe table's rate basis is run 1's canary, 0.89 s, whose correctness half is recovered from the run-1 driver capture (`944395f0…`) — not from the chunks, which do not carry it; run 4's canary was checked at the driver and re-verified by `report`*; and the sentence: *in any artifact `report` can write, `observed` equals `checked_against` by construction; the independent evidence of what the engine answered is the manifested `canary.json` and `canary.log`*; (4) both clocks per stage; (5) **where each driver ran**: runs 1–3 as detached tmux sessions launched from a tool call with nobody attached (the implementer's exact command), run 4 attended by the author; (6) the four-way diff; (7) the losses, in these words: run 1's smoke chunks were overwritten at 14:08 and their fenced block is lost (search stated by name and content); nothing else is lost; (8) the two `canary.log` formats; the traci `Retrying in 1 seconds` lines; the interim packet `P7.2b-e1.3-e1.4.md` referenced, not duplicated; (9) the amendments and blocks it was written against, by letter and time; (10) the AI-assistance record, with *human verification performed* naming the author's run 4 and the coordinator's rounds by their Decisions-Log rows; (11) *what P7.3 will assume*.
