...
# ⛔ AMENDMENT J — 2026-09-17, on F1's pilot at `af5f883` and the pre-flight: the pool, a full real episode, resume and destruction all WORK and the pilot is ACCEPTED as the mechanics check — but PROVENANCE IS UNCHECKED at every consumer and the driver has two signal gaps that leave a consumed token with no end state. One fix round, then the pilot re-run at the fix commit, then the stage-1 token

**Verified by the coordinator from disk:** Amendment I an ancestor of `af5f883`; no trailer; tree clean; `af5f883` **not on origin** (origin at `a0327ce`); whole suite **2058 passed / 94 skipped, exit 0** (coordinator's run); guards 0/0/0; no `p7_3a*` file in `docs/data/`; `report` pointed at the real pilot directory refuses at the missing `canary.json` and creates no out-dir (coordinator's run). All 16 canonical pilot chunks and the 17 other real chunks record `git_commit a0327ce, git_dirty: true` — **the pilot ran on an uncommitted tree.** **Three of the pilot's evidence files lived only on `/tmp`, which is tmpfs on this machine** (run 2's transcript, chunks and full log; the single-worker log; the destruction probe's transcript, which holds the packet's 0.76 s and the 15/1 run); the coordinator copied them unmodified to `output/p7_3a_runs/preflight_pilot_{run2,run2.txt,run2_full.log,single,single.log,resume_probe.txt}` with sha256 recorded in the Decisions Log. **The pre-flight** (contract reviewer on a separate model after the first attempt hit a rate limit during setup and was cleaned up; driver copy outside a clean worktree at `af5f883`, real inputs untouched, verified after) returned **PASS-WITH-FINDINGS, 0 blocking / 1 major / 9 minor**: the no-token and non-leader refusals create nothing; a failed canary correctness half (falsified on the real engine) leaves the token byte-identical; H2's skip branch and H1's stop run twice with `logs/a17f.log` appended and the corpus unchanged; SIGINT delivered the way tmux delivers it exits 130 with `CAMPAIGN INTERRUPTED` and no orphan; resume by content on copies of the real chunks reuses 16, a truncated chunk and a chunk with a tampered checkpoint digest are each moved aside and re-rolled **clean at `af5f883`**; the header's 2.003 s/cell, 7.16×, ≈ 40 min and ≈ 2.6 h recompute from run 1's transcript.

## J1 — MAJOR (the reviewer's MAJ-1, extended by the coordinator): provenance is recorded on every chunk and checked by NOTHING
`chunk_is_reusable` and `report` never read a chunk's `git_commit` or `git_dirty`; the artifact's top-level `git_dirty` is `report`'s own, taken at write time. So §7's gate — *the zero-shot artifact's provenance is clean* — can read clean over cells rolled on a dirty tree. **And the helper fails open:** `materialise_draws._git_commit` returns `dirty = False` whenever `git status` cannot run, so an unmeasured tree reads as a clean one. **Required:**
- (a) `transfer_curve` measures provenance **strictly**: `git rev-parse HEAD` and `git status --porcelain` from the module's tree, and a failure of either **raises** — P7.3a does not reuse the fail-open helper (P7.2a's merged artifacts are not touched by this).
- (b) `validate_cell_payload` refuses `git_dirty is not False` and a missing or non-40-hex `git_commit`.
- (c) **Commits may differ only by documentation.** `chunk_is_reusable` returns `False`, and `report` refuses, when `git diff --name-only <chunk commit> <HEAD>` lists any path outside `docs/`; so a chunk rolled before the stage-1 artifact's own commit is still reusable, and a chunk rolled by different code is not. The artifact records the set of chunk commits per stage.
- (d) **The driver refuses before the token when its tree is dirty** (`git -C "$WORK_TREE" status --porcelain` non-empty), naming the paths.
- (e) **Operational, coordinator's ruling: the campaign runs from a dedicated worktree** `/home/filip/rltraffic-p73a-run`, detached at the reviewed commit, that no session edits; the implementer's worktree stays the place where code is written. A multi-hour stage must not share a tree with an active session, because one untracked file makes every later chunk dirty.
*Tests:* a dirty chunk refused by `validate_cell_payload`; a chunk from a commit whose diff to HEAD touches `offline/` not reusable and refused by `report`; a docs-only difference reusable; a failing `git` raises. *Mutations:* each check removed → dies; the fail-open helper substituted → the failing-`git` test dies.

## J2 — MAJOR (reviewer min-3, promoted): `SIGHUP` is not trapped, and a closed tmux pane or a dropped SSH session sends exactly that
Measured: SIGHUP to the group during A17(f) exits 129 with the token consumed, `canary.json` and the logs written, and **neither `FAILED` nor `COMPLETE`** — the end state the header's trap-before-token rule exists to prevent. **Required:** `trap on_signal INT TERM HUP`. *T9:* the trap line names all three.

## J3 — Minor, same round, cheap
- **(reviewer min-2) A shell that starts with SIGINT ignored cannot trap it** (bash does not let a non-interactive shell trap a signal ignored on entry), so Ctrl-C does nothing and the pool keeps writing. **Required:** a pre-token refusal when `/proc/$$/status` `SigIgn` has the INT bit set, fail-closed like the group-leader check, naming the cause. The header's sentence *"`bash script.sh &` does not [lead its group]"* is corrected to what was measured (a plain `&` from a non-interactive shell did lead its group; `set +m` produced the non-leader).
- **(min-4)** A failed canary correctness half prints its own refusal line — *nothing has been consumed* — and exits 2, not `fail()`'s 1: `if ! CANARY_LINE=$(…); then …; exit 2; fi`.
- **(min-6)** `run_cell` loads the calibration through `data_dir`, as the checkpoint pins already do.
- **(min-7)** `run_pilot` prints the canary line before it checks it, as the `canary` subcommand does since H8.
- **(min-1, min-5)** no change: T9 strips comments before grepping; `canary.json`'s format version names the shared P7.2b helper that writes it.

## J4 — The packet addendum corrects two statements against the files now on disk
**(min-8)** §15.8(3) lists three pilot canaries; the transcripts hold **five** — 0.92, 0.79, 0.76, 0.73, 0.77 — with the file and entry of each. **(min-9)** *"one cell at one worker also retries twice"*: the retry count is on disk (`preflight_pilot_single.log`, 2 lines for 1 cell), the worker count is not; state what the file shows. **Standing rule, from this round: no evidence of a P7.3a run is written to `/tmp`.** The handoff states `/tmp` is tmpfs here; three evidence files were one reboot from gone. Every transcript, capture and work directory goes under `output/p7_3a_runs/`.

## J5 — The pilot is RE-RUN at the fix commit, from the dedicated worktree, and that run is the header's rate basis
The 16-cell rate was measured on `a0327ce` plus uncommitted edits; the pre-flight re-rolled single cells only. After J1–J4: `/home/filip/rltraffic-p73a-run` detached at the fix commit, `pilot --workers 12` into `output/p7_3a_runs/preflight_pilot_fix/` with its transcript beside it — every chunk must record `git_dirty: false` and the fix commit — and **one interrupted run first:** the same pilot started under `setsid`, `SIGTERM` to its group after ≈ 10 s of pooled rolling, then `pgrep` empty, no partial chunk file (atomic writes), and a re-run completing with the survivors reused. That closes the one path neither the implementer nor the reviewer executed: a signal while spawn workers are live. The header cites this run — its canary, date, n and commit — and it is the F3 comparison basis (packet Q3).

## J6 — Packet questions
**Q1 (the traci retry):** recorded as a cost (A10, now measured at 2 per SUMO start); `envs/sumo_env.py` is frozen; no separate finding. **Q2 (run 2 in `/tmp`):** answered by J4's rule; the coordinator has already copied it. **Q3 (F3's basis):** J5's run.

## J7 — Not executed by anyone, and where it is closed
