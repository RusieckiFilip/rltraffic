...
| 26 | **Automate the checklist drift check** | §6's header carries it as a shell one-liner; a test would make it mechanical rather than remembered. It found 3 violations the day it was written (P2.2-draws, P2.6, P3 all had Return Packets on `main` and no ticked box). | ~20 lines + a test | Whoever next touches `tests/`; cheap and it closes a failure that has occurred twice in opposite directions |
