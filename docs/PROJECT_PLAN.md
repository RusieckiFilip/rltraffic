...
| 2026-08-03 | **`scripts/` is the least-protected part of the frozen set, and its only protection is circular.** Found by the implementer session. CLAUDE.md freezes `scripts/**`, but the permission system denies exactly one path in it — `Edit(scripts/claude_guard.sh)`. Everything else in `scripts/` is governed solely by `claude_guard.sh`, a script living inside the directory it protects. Compounding fact already on record: the guard reads `git status`, so a clean exit after a commit proves nothing about what was committed. Today's exposure is **prospective, not actual** — `scripts/` contains exactly three files (verified 2026-08-03) and the other two are the intended `FROZEN_EXCEPTIONS`, so no unprotected frozen script exists *right now*; the hole opens the moment a fourth script is added | Circularity is not defence-in-depth. **Remedy open** pending a ruling: naming each frozen script individually is a no-op today and relies on humans remembering to extend the list, whereas a glob would give prospective cover at the cost of making the two tunable scripts human-edit-only. The experiment above proves there is no third option that keeps both |
...
**Open ruling awaiting the user:** how to close the `scripts/` protection gap — individual denies
(no-op today, relies on humans extending the list) vs a glob deny (prospective cover, makes
`check_english.sh` / `check_test_hygiene.sh` human-edit-only). The permission experiment proves no
option keeps both. Settings changes ship via `docs/patches/` because `Edit(.claude/**)` is denied.
