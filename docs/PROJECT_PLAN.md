...
**Version:** 0.6 · **Last updated:** 2026-08-03 · **Maintained in:** Master Coordination Chat
...
**Closed:** P0 (except P0.4–P0.7) · P1 · P2.0 / P2.0b / P2.0c. Two mechanical guards live (frozen-file,
test-hygiene). **Suite: 249 green as of 2026-08-02** (P0.3-fix session) — not re-run since, so treat it
as hearsay until a session runs it and says so.

**Immediate queue, in order:**
1. **P0.4** — fill / freeze / hash `PREREGISTRATION.md`, cut tag `v0.1-prereg`. The file exists and is
   committed (`e205e92`) but is **still a template**: its header reads `**Registered:** <YYYY-MM-DD>`
   and `**Doc sha256:** <fill after freezing>`, and `git tag -l` is empty (verified 2026-08-03).
   **This is the only open item whose value decays with time** — once a corpus exists, a
   "pre-registration" timestamped after the data is worthless, and claiming it would be misconduct.
2. **P0.5 / P0.6 / P0.7** — one bundled brief closing review findings N1 / N5 / N7 / N8.
...
**⏳ Awaiting a human action (not a decision):** apply `docs/patches/settings_scripts_glob_deny.patch`
(`git apply docs/patches/settings_scripts_glob_deny.patch`, then **restart the Claude Code session** —
permissions are read at session start). Until it is applied, `scripts/` is still protected only by a
script inside itself. Post-application, the Master chat owes one on-repo falsification run: attempt an
edit to a new file under `scripts/` and confirm it is denied against the *real* settings, not a
simulated rule set.
...
`--local-reward-fn queue_length --global-reward-weight 0.0`.
