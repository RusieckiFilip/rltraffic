...
- [ ] **P0.6** Commit the benchmark config to `experiments/configs/` — reviewer N5. The 199.2 s / 50.2 s headline numbers rest on a config that lives only in a session scratchpad. A repo whose output is a paper cannot have its timing evidence be unreproducible. No authorisation needed (a *new* config is explicitly permitted)
