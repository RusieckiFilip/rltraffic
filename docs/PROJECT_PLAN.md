...
- [x] **P0.3** ✅ DIAGNOSED 2026-07-27 (`docs/notes/P0.3_spawn_attempt.md`, commit `38b3d4a`). **The July deadlock was NEVER a deadlock — it was thread oversubscription.** Benchmark (reduced p0_baselines, 6 cells, serialized): unpinned workers=3 is **0.29× (3.4× SLOWER than sequential)** — 48–96 torch OMP threads contending on 16 cores, which pegs CPU while making no progress = the "frozen terminal" symptom. Spawn was irrelevant; the patch fixed the wrong problem. All 3 note hypotheses + Master-chat's H4 (parent-torch-before-fork) refuted by experiment. **Real fix = `torch.set_num_threads(1)` (P0.3-fix / Brief #2b), NOT the spawn patch.**: `ProcessPoolExecutor(..., mp_context=mp.get_context("spawn"))` — fixes fork deadlock with torch/CityFlow C++ locks. Verify: smoke.json with seeds [7,8,9], `--workers 3`
- [ ] **P0.3-fix** ⚡ Thread-pinning in `experiments/runner.py` (`torch.set_num_threads(1)` per worker) — touches a FROZEN file → own branch `task/p0.3-thread-pinning` + independent review. Measured payoff: **workers=6 pinned = 5.80×** (vs 0.28× unpinned), AND **workers=1 pinned = 1.37×** — i.e. pinning speeds up EVEN sequential runs 37% (torch's default pool hurts small MLPs). Verify per-worker with `torch.get_num_threads()==1` inside `run_cell` *(Brief #2b)*
...
**Active task:** P2.0 FlowRandomizer (Brief #2a) — **blocking** (corpus is information-free without it). In parallel: P0.3-fix (Brief #2b, thread-pinning, frozen-file task) before P5: the corpus carries no information without it (determinism confirmed 2026-07-27). Budget and scope frozen (P2.0b/c); compute is not the constraint.
