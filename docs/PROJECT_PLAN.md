...
**Phases P0, P1, P2.0 CLOSED.** Suite 249 green. Two mechanical guards live (frozen files, test hygiene).
**Next:** P0.5 (liveness docstring — needs a new frozen-file authorisation) · P0.6 (benchmark config) · P0.7 (test hardening) · **P2.5 fixed-time (Tier 1 of the ladder, Brief #2c — not yet written)** · P2.4 corpus linter · P2.1 MAPPO convergence.
**Still owed:** cut the `v0.1-prereg` tag (§P0.4, unblocked since the reward decision, still not cut); sweep every cross-session ratio in this document.
**Inherited by every later task:** `experiments/runner.py` is no longer byte-identical to main's frozen copy (one authorised delta, `docs/patches/runner_thread_pinning.patch`); parallelism is safe *because of* the pin; corpus numerics are on the pinned side of a float-reduction boundary.
