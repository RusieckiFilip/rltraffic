...
- [ ] **P0.3** Patch `experiments/runner.py`: `ProcessPoolExecutor(..., mp_context=mp.get_context("spawn"))` — fixes fork deadlock with torch/CityFlow C++ locks. Verify: smoke.json with seeds [7,8,9], `--workers 3`
...
### Phase P1 — Trajectory Logger  ← **CURRENT: Sub-chat Brief #1**
- [ ] **P1.1** `offline/trajectory_logger.py`: episode recorder wrapping the act/step/observe loop (wrapper/callback pattern, zero env modifications)
- [ ] **P1.2** `offline/collect.py`: CLI — behavior policy × scenario × episodes × seeds → dataset dir
- [ ] **P1.3** `tests/test_trajectory_logger.py`: shape/roundtrip/determinism tests
- [ ] **P1.4** Return Packet reviewed in Master chat; plan updated
- [ ] **P1.5** Independent review sub-chat (critical path): diff + brief + contracts → discrepancy report; merge `task/p1-logger` only after pass
...
**Active task:** P1 (Trajectory Logger) — Brief #1 in play; start its sub-chat now (its tests use a fake env, no simulator needed).
**Quick fixes pending:** P0.3 spawn patch; **Brief #1 v2 is FROZEN — run it, no further review rounds on the document**; drop `CONTRACTS_C6_v1.1.md` into `docs/CONTRACTS.md` and the two guard files into the repo before starting; **P0.4 pre-registration UNBLOCKED** (reward decided 2026-07-12) — tag `v0.1-prereg` now. Mentor email no longer needed (free hand given; credit deferred to P10).
**Next in queue:** Sub-chat Brief #2a (P2.0 FlowRandomizer) + P2.0b budget sheet — both BEFORE any corpus collection; then P2.1+.
